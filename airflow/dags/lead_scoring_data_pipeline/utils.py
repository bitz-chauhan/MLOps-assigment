...
def build_dbs():
    ...
    SAMPLE USAGE
        build_dbs()
    '''
    print("checking path >> " + DB_PATH + DB_FILE_NAME)
...
def load_data_into_db():
    ...
    df_lead_scoring.to_sql(name='loaded_data', con=con, if_exists='replace', index=False)
...
def map_city_tier():
    ...
    df_lead_scoring = pd.read_sql('select * from loaded_data', con)
    ...
    df_lead_scoring.to_sql(name='city_tier_mapped', con=con, if_exists='replace', index=False)
